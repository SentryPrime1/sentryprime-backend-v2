...
const initDB = async () => {
  try {
    ...
    initializeAltTextAIRoutes(db, {
      ...
      notifications: { enableInApp: true, enableEmail: false, enableWebhooks: false }
    });
...
app.get('/api/migrate', async (req, res, next) => {
  try {
    ...
    if (!dbInitialized) throw new Error('Database connection could not be established');
    await runMigration();
  ...
  try {
    ...
    (async () => {
      try {
        ...
        console.log(`Scan completed and saved to DB for scan ID: ${scan.id}`);
...
const gracefulShutdown = async (signal) => {
  ...
  server.close(async () => {
    if (dbInitialized) await closeDatabase();
    process.exit(0);
  });
};
